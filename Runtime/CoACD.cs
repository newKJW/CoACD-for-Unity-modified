...
public unsafe class CoACD : MonoBehaviour
{
    ...
    public UnityEngine.Object targetFBX;
	...
	public unsafe struct Parameters
	{
		public static Parameters Init() => new Parameters() {
			threshold = 0.012, preprocessMode = PreprocessMode.Off, preprocessResolution = 100, sampleResolution = 10000,
			mctsNodes = 30, mctsIteration    = 300, mctsMaxDepth = 4, pca = false, merge = true, maxConvexHull = -1, seed = 0
		};
	...
	public List<Mesh> RunACD(Mesh mesh)
	{
		...
		fixed (double* vptr = v) {
			fixed (int* fptr = unityF) {
				...
				Debug.Log("Run started at: " + DateTime.Now.ToString());
                ...
                Debug.Log("Run end at: " + DateTime.Now.ToString());
	...
	[Button]
#else
[ContextMenu("Calculate Colliders From Target")]
#endif
			...
			{
				{
					...
					if (string.IsNullOrEmpty(path)) {
						path = PrefabPathUtil.GetPrefabAssetPath(this);
					}

					if (string.IsNullOrEmpty(path) || !path.StartsWith("Assets/"))
					{
						if (!AssetDatabase.IsValidFolder("Assets/CoACD_NonPrefabColliderGarage"))
						{
							Debug.Log("i executed!!");
							string guid = AssetDatabase.CreateFolder("Assets", "CoACD_NonPrefabColliderGarage");
							AssetDatabase.GUIDToAssetPath(guid);
						}
                        path = "Assets/CoACD_NonPrefabColliderGarage/" + this.gameObject.name + ".dummy";
					}
				}
...
[ContextMenu("Calculate Colliders for Hierarchy")]
	...
	[Button]
#else
[ContextMenu("Calculate Colliders from FBXfile")]
#endif
	void CalculateCollidersFromFBX()
	{
		var  parameters    = this.parameters;
		var  baseTransform = transform;
		// Mesh mesh          = null;
		var  path = AssetDatabase.GetAssetPath(targetFBX);
		EditorUtility.DisplayProgressBar("Calculating Colliders of FBX", "Collectig meshes...", 0.1f);
		var originalMeshes    = new List<Mesh>();
		var meshesToDecompose = new List<Mesh>();
		{
			if (string.IsNullOrEmpty(path))
			{
				EditorUtility.ClearProgressBar();
				EditorUtility.DisplayDialog("Error",
					$"Could not Find the targetFBX file in Directory.\nTry Reload the FBX file to targetFBX parameter.", "Ok");
			return;
			}
			...
			{
				EditorUtility.ClearProgressBar();
				EditorUtility.DisplayProgressBar("Calculating Colliders of FBX", $"Collecting meshes... ({c++}/{meshes.Count})",
					Mathf.Lerp(0.2f, 0.3f, Mathf.InverseLerp(1, meshes.Count + 1, c)));
		...
		EditorUtility.ClearProgressBar();
		EditorUtility.DisplayProgressBar("Calculating Colliders of FBX", "Combining meshes...", 0.4f);
		var decomposedMeshes = new List<Mesh>();
		{
			var c = 1;
			foreach (var meshToDecompose in meshesToDecompose) {
				EditorUtility.ClearProgressBar();
				EditorUtility.DisplayProgressBar("Calculating Colliders of FBX", $"Processing mesh... ({c++}/{meshesToDecompose.Count}) (this can take a while)",
					Mathf.Lerp(0.5f, 0.7f, Mathf.InverseLerp(1, meshesToDecompose.Count + 1, c)));
				var tempMeshes = RunACD(meshToDecompose);
				decomposedMeshes.AddRange(tempMeshes);
			}
		...
		EditorUtility.DisplayProgressBar("Calculating Colliders of FBX", "Storing submeshes...", 0.8f);
		if (_colliderData == null) {
			path          = Path.ChangeExtension(path, null) + $"_Colliders.asset";
			path          = AssetDatabase.GenerateUniqueAssetPath(path);
			_colliderData = CoACDColliderData.CreateAsset(path, parameters, decomposedMeshes.ToArray(), originalMeshes.ToArray());
	...
	void RemoveMeshColliders()
    {
        ...
        GetComponents<MeshCollider>(colliders);
...
public static class PrefabPathUtil
{
    public static string GetPrefabAssetPath(Component c)
    {
        ...
        if (stage != null)
            return stage.assetPath; // "Assets/Prefabs/My.prefab"
